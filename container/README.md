...
> [!TIP]
> Available options include the following, these names are recognised by the `app_set_config 90 mod` command in [`hlds.txt`](hlds.txt).
>
...
>
> To install a specific sub version, such as a beta, you can utilise the `FLAG` environment variable to pass arbitrary command flags to SteamCMD. For example, `export FLAG=-beta steam_legacy` can be used to get a [Pre-25th Anniversary build](https://www.half-life.com/en/halflife25) of some titles.
